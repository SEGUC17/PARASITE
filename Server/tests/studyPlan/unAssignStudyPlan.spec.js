...
describe('/PATCH one of my study plans to assigned = true', function () {
  ...
  it('it should PATCH a user\'s own Study Plan to assigned', function (done) {
    // Logging in
    chai.request(server).
    post('/api/signIn').
    send({
    ...
    chai.request(server).
      patch('/api//study-plan/unAssignStudyPlan/' + johnDoe.username + '/' + studyPlan._id).
  ...
  it('it should PATCH a user\'s child\'s own Study Plan to assigned', function (done) {
    // Logging in
    chai.request(server).
    post('/api/signIn').
    send({
    ...
    chai.request(server).
      patch('/api//study-plan/unAssignStudyPlan/' + johnny.username + '/' + studyPlan._id).
      set('Authorization', signinData.body.token).
      end(function (error, res) {
        if (error) {
            console.log(error);
  ...
  it('it should not PATCH another user\'s Study Plan to assigned who isn\'t the user or their child', function (done) {
    // Logging in
    chai.request(server).
    post('/api/signIn').
    send({
    ...
    chai.request(server).
    patch('/api//study-plan/unAssignStudyPlan/' + janeDoe.username + '/' + studyPlan._id).
      set('Authorization', signinData.body.token).
      end(function (error, res) {
        if (error) {
            console.log(error);
